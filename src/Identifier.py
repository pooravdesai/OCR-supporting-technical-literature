#DO NOT COPY TO MAIN PROJECT FOLDER
...
class Identifier:
    ...
    def identify(self,image,bbox):
        ...
        for x,y in zip(bbox,prediction):
            c = chr(int(str(y[2:]), 16))
            if c == '\x92':
                c = '\''
            elif c == '−' or c == '-' or c == '—':
                c = '-'
            x.append(c)                     #will work with unicode
            #x.append(chr(y))
        return bbox
    
    def identifyMath(self,image,bbox):
        inputx = self.prepareDataMath(image,bbox)
        prediction = self.predictMath(inputx)
        for x,y in zip(bbox,prediction):
            c = chr(int(str(y[2:]), 16))
            if c == '\x92':
                c = '\''
            x.append(c)                   #will work with unicode
            #x.append(chr(int(str(y), 16)))                     #will work with unicode
            #x.append(y)
        return bbox


    def predict(self,inputx):
        model = joblib.load('model_text_math.pkl')                #use the correct model
        prediction = model.predict(inputx)
        return prediction

    def predictMath(self,inputx):
        model = joblib.load('model_math.pkl')                #use the correct model
        prediction = model.predict(inputx)
        return prediction


    def prepareDataMath(self,image,bbox):
        inputx = np.empty((len(bbox),10000),dtype = image.dtype)
        count = 0
        for boundary in bbox:
            a, left,top,right,bottom = boundary
            character_image = self.bbox_image_preprocess(image[top:bottom+1,left:right+1])
            #cv2.imwrite('img' + str(count) + '.jpg', character_image)
